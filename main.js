...
async function getData(ticker, start, end) {
	try {
		...
		if (
			startDate == undefined ||
			new Date(startPeriodDate) < new Date(startDate)
		) {
			let prevExp = await db.query('CALL GetPreviousExpDate(?);', [start]);
		...
		if (retDate == undefined || retDate <= endDate) {
			let expDate = await db.query('CALL GetNextExpDate(?,?);', [ticker, end]);
